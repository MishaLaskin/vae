...
class GoalImageEnv:
    ...
    def normalized_render(self):
        ...
        _, z_obs, _, _ = self.model1(img)
        _, z_goal, _, _ = self.model2(img)

        z_obs = z_obs.squeeze(0).detach().cpu().numpy()
        z_goal = z_goal.squeeze(0).detach().cpu().numpy()
    ...
    def reset(self):
        self.steps = 0
        self.dm_env.reset()
        _, self.desired_goal = self.normalized_render()
        self.dm_env.reset()
        obs, achieved_goal = self.normalized_render()
...
class Stacker(GoalImageEnv):
    ...
    def __init__(self,
        ...
        super().__init__(env_name='stacker',
                         mode='just_place',
                         act_dim=None,
        ...
        path1 = '/home/misha/research/baselines/vae/saved_models/just_place_vae.pth'
        path2 = '/home/misha/research/baselines/vae/saved_models/two_blocks_vae.pth'

        self.model1.load_state_dict(torch.load(path1))
        self.model1 = self.model1.to(self.device)
        self.model2.load_state_dict(torch.load(path1))
        self.model2 = self.model1.to(self.device)
